...
class CannonView @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP) constructor(context: Context, attrs: AttributeSet?) : SurfaceView(context, attrs), SurfaceHolder.Callback {
    ...
    private var gameOver: Boolean = false // is the game over? = false
    private var timeLeft: Double = 0.0 // time remaining in seconds = 0.0
    private var shotsFired: Int = 0 // shots the user has fired = 0
    private var totalElapsedTime: Double = 0.0 // elapsed seconds = 0.0
...
// such as when it's first added to the View hierarchy
    ...
    fun newGame() { // construct a new Cannon
        cannon = Cannon(this,
...
// to the right
            var x = (TARGET_WIDTH_PERCENT + TARGET_SPACING_PERCENT)

            targetX += (x * screenWidth).toInt()
...
// already on the screen
    fun alignAndFireCannonball(event: MotionEvent) { // get the location of the touch in this view
        val touchPoint = Point(event.x.toInt(),
                event.y.toInt())
...
// on the y-axis
    ...
    private fun showGameOverDialog(messageId: Int) { // DialogFragment to display game stats and start new game
        val gameResult: DialogFragment = object : DialogFragment() {
            // create an AlertDialog and return it
            override fun onCreateDialog(bundle: Bundle): Dialog { // create dialog displaying String resource for messageId
    ...
    fun drawGameElements(canvas: Canvas?) { // clear the background
        ...
        if (cannon!!.cannonball != null &&
                cannon!!.cannonball!!.isOnScreen) cannon!!.cannonball!!.draw(canvas)
...
// and handles the collisions
    fun testForCollisions() { // remove any of the targets that the Cannonball
// collides with
        if (cannon!!.cannonball != null &&
                cannon!!.cannonball!!.isOnScreen) {
            var n = 0
    ...
    override fun surfaceDestroyed(holder: SurfaceHolder) { // ensure that thread terminates properly
        var retry = true
    ...
    override fun onTouchEvent(e: MotionEvent): Boolean { // get int representing the type of action which caused this event
        val action = e.action
        // the user touched the screen or dragged along the screen
        if (action == MotionEvent.ACTION_DOWN ||
                action == MotionEvent.ACTION_MOVE) { // fire the cannonball toward the touch point
    ...
    private inner class CannonThread(// for manipulating canvas
        ...
        override fun run() {
            ...
            while (threadIsRunning) {
                try { // get Canvas for exclusive drawing from this thread
                ...
                } finally { // display canvas's contents on the CannonView
// and enable other threads to use the Canvas
    ...
    private fun hideSystemBars() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) systemUiVisibility = View.SYSTEM_UI_FLAG_LAYOUT_STABLE or
